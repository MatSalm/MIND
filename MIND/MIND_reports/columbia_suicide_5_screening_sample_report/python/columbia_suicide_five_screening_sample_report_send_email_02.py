...
matching_files = list(current_directory.glob(f'columbia_screening_random_sample_{start_date.strftime("%Y-%m-%d")}_to_{end_date.strftime("%Y-%m-%d")}.xlsx'))
...
msg['From'] = smtp_email
msg['To'] = to_email
msg['Subject'] = f'Columbia Screening Random Sample Report {start_date.strftime("%Y-%m-%d")} to {end_date.strftime("%Y-%m-%d")}'
...
<html>
  <body style="font-family: 'Segoe UI', sans-serif; color: #242424;">
    <p>Hello,</p>
    <p style="margin-left: 40px;">Attached is the Columbia Screening Random Sample Report for {start_date.strftime("%Y-%m-%d")} to {end_date.strftime("%Y-%m-%d")}.</p>
    <p>Thank you,</p>
