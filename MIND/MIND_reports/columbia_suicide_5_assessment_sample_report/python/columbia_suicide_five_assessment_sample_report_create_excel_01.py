...
try:
    ...
    if sampled_df is None or sampled_df.empty:
        raise ValueError("Sampled DataFrame is None or empty.")
    print(f"Loaded sampled DataFrame with {len(sampled_df)} rows.")
...
try:
    ...
    print(f"Loaded date parameters: start_date={start_date}, end_date={end_date}")
except Exception as e:
...
header_text = f'Columbia Assessment Sample {start_date.strftime("%Y-%m-%d")} to {end_date.strftime("%Y-%m-%d")}'
...
output_filename = f'columbia_assessment_sample_{start_date.strftime("%Y-%m-%d")}_to_{end_date.strftime("%Y-%m-%d")}.xlsx'
try:
